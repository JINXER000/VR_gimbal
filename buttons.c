...
extern int workstate,stablecnt,shoudongflag;
extern long tmcount;
extern float imuangle[3],lastangle[3],lastheadangle[3],headangle[3];
int ppwm0;
...
ButtonsInit(void)
{
	...
	ROM_SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOM);
	 ROM_GPIOPinTypeGPIOOutput(GPIO_PORTM_BASE, GPIO_PIN_0|GPIO_PIN_1);
...
SysTickIntHandler(void)
{
    ...
    if(BUTTON_PRESSED(USR_SW1, ui8Buttons, ui8ButtonsChanged))
    {
		...
		if(tmcount==100)
		{
					...
					{				
							if((((lastangle[0]-imuangle[0])<3)&&((lastangle[0]-imuangle[0])>-3))&&(((lastangle[2]-imuangle[2])<3)&&((lastangle[2]-imuangle[2])>-3)))
						{
							stablecnt++;
		...
		tmcount=0;
				lastangle[0]=imuangle[0];
				lastangle[1]=imuangle[1];
			  lastangle[2]=imuangle[2];
				lastheadangle[0]=headangle[0];
					lastheadangle[2]=headangle[2];
